(ns riceops.store
  "SSoT for the rice-paddy operations coordinator, behind a `Store`
  ...
  (test/riceops/store_contract_test.cljc).
  ...
  (e.g. `cerealops.store`'s `:field/payload`).

  The append-only audit ledger (`ledger`/`append-ledger!`) is this actor's
  core missing plumbing until now: `riceops.operation`'s `:commit`/`:hold`
  graph nodes append every committed/held/approval-rejected decision fact
  here, so a paddy field's operating history (every `:log-field-record` /
...
(defprotocol Store
  ...
  (add-field [store field-id field-data]
    "Register or update a paddy field in the store. Used by tests,
