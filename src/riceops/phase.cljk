(ns riceops.phase
  "Phase gate for the rice-paddy operations coordinator rollout (0->3
  maturity). Mirrors `cerealops.phase` (cloud-itonami-isic-0111) in shape,
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`riceops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [riceops.governor :as governor]))
