(ns riceops.operation-graph-test
  "Integration tests for `riceops.operation/build` -- proves the REAL
...
(defn- store-with-field []
  (store/mem-store {:initial-fields {"paddy-1" {:crop "rice" :acreage 15}}}))
  ...
  (testing "phase-3 is full autonomy -- a clean, registered-field proposal
    ...
    (let [s (store-with-field)
      ...
      (let [result (exec actor "t-commit"
                         {:op :log-field-record :field-id "paddy-1"
                          :acreage 15 :crop "rice" :record-type "planting"})
            state (:state result)]
...
(deftest hard-hold-irrigation-equipment-permanently-blocked
  (testing ":operate-irrigation-equipment is a permanent, un-overridable
            block regardless of phase, proven end-to-end through the
            compiled graph -- distinct from the shared
            :operate-field-equipment block, this is riceops' own
            irrigation-specific addition to blocked-ops"
    (let [s (store-with-field)
          actor (operation/build s)
          result (exec actor "t-irrigation"
                       {:op :operate-irrigation-equipment :field-id "paddy-1"})]
...
(deftest escalate-then-approve-commits-and-genuinely-consults-advisor
  (testing ":flag-crop-health-concern ALWAYS escalates (Governor
            ...
            hardcoded in riceops.operation) threads through
    ...
    (let [distinctive-concern (str "TEST-CONCERN-" (rand-int 1000000000))
      ...
      (let [held (exec actor "t-escalate"
                       {:op :flag-crop-health-concern :field-id "paddy-1"
  ...
  (testing "a human agronomist rejecting an escalated
    ...
    (let [s (store-with-field)
          ...
          _held (exec actor "t-reject"
                      {:op :flag-crop-health-concern :field-id "paddy-1"
                       :concern "possible blast disease"})
  ...
  (testing "phase-0 (default-phase) is simulation-only -- no proposal ever
    ...
    (let [s (store-with-field)
          ...
          held (exec actor "t-phase0"
                     {:op :log-field-record :field-id "paddy-1" :acreage 15}
