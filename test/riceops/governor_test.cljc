(ns riceops.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [riceops.governor :as gov]
...
(deftest hard-violations-effect-not-propose
  (testing "Hard violation: effect is not :propose"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
...
(deftest hard-violations-irrigation-equipment-blocked
  (testing "Hard violation: direct irrigation-equipment operation (flooding/drainage valves, pumps) is permanently blocked"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
          prop {:op :operate-irrigation-equipment :effect :propose}
          verdict (gov/check req nil prop s)]
      (is (:hard? verdict))
      (is (some #(= :equipment-or-pesticide-decision-blocked (:rule %)) (:violations verdict))))))
...
(deftest hard-violations-water-level-invalid
  (testing "Hard violation: negative water-level"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
          prop {:op :log-field-record :effect :propose :acreage 80 :water-level -1 :confidence 0.9}
          verdict (gov/check req nil prop s)]
      (is (:hard? verdict))
      (is (some #(= :water-level-invalid (:rule %)) (:violations verdict))))))
...
(deftest ok-field-logging
  (testing "OK: valid field record logging with a registered field"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
          prop {:op :log-field-record :effect :propose :acreage 80 :water-level 5 :confidence 0.9}
          verdict (gov/check req nil prop s)]
      (is (:ok? verdict))
      (is (not (:hard? verdict)))
      (is (not (:escalate? verdict))))))

(deftest ok-field-logging-zero-water-level
  (testing "OK: zero water-level is a valid (drained/dry) paddy observation"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
          prop {:op :log-field-record :effect :propose :acreage 80 :water-level 0 :confidence 0.9}
          verdict (gov/check req nil prop s)]
      (is (:ok? verdict))
      (is (not (:hard? verdict))))))

(deftest escalation-crop-health-concern
  (testing "Escalation: crop pest/disease/blast-fungus/drought-stress concern ALWAYS escalates, even at high confidence"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
          prop {:op :flag-crop-health-concern :effect :propose
                :concern "いもち病（blast fungus）の可能性" :confidence 0.95}
          verdict (gov/check req nil prop s)]
      (is (not (:hard? verdict)))
      (is (:escalate? verdict))
...
(deftest ok-schedule-field-operation
  (testing "OK: scheduling a field operation (planting/flooding-drainage/harvest) is a routine coordination op"
    (let [field {:id "paddy-001" :name "Test Farm North Paddy"}
          s (store/mem-store {:initial-fields {"paddy-001" field}})
          req {:field-id "paddy-001"}
